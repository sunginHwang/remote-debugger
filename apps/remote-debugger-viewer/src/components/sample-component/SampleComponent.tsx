...
export function SampleComponent({
  ...
  const handleSaveToServer = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5543/rrweb/events", {
        method: "POST",
        body: JSON.stringify({
      ...
      console.log(
        `✅ ${result.saved || events.length}개 이벤트가 pack되어 전송되었습니다. Session ID: ${result.sessionId || sessionId}`
      );
    } catch (error) {
  ...
  return (
    <>
      ...
      <button onClick={onRunReplay}>리플레이 재생하기</button>
